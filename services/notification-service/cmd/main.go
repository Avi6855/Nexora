...
func main() {
	...
	cfg := config.LoadServiceConfig("notification-service", 8090, 8190)
	...
	notifRepo := repository.NewCassandraNotificationRepository(session)

	producer, err := events.NewKafkaProducer(cfg.Kafka.Brokers, "nexora.notification.events", logger)
	if err != nil {
