...
import (
	...
	"github.com/nexora/nexora/services/dispute-service/internal/transport"
...
func main() {
	...
	cfg := config.LoadServiceConfig("dispute-service", 8098, 8198)
	...
	// review, review elapsed → customer-favour resolution with a real refund).
	...
	go func() {
		...
		for {
			...
			case <-tickerCtx.Done():
				return
			case <-t.C:
				advanced, err := disputes.ProcessDeadlines(tickerCtx)
				if err != nil {
					logger.Error().Err(err).Msg("dispute deadline tick failed")
				} else if advanced > 0 {
					logger.Info().Int("advanced", advanced).Msg("dispute deadline tick advanced cases")
				}
			}
		}
	}()
	...
	router.Use(telemetry.HTTPMetrics(metricsRegistry, "dispute-service"))
	...
	logger.Info().Msg("shutting down dispute-service...")
	...
	logger.Info().Msg("dispute-service stopped")
}
