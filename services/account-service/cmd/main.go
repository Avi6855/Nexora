...
import (
	...
	"github.com/nexora/nexora/services/account-service/internal/transport"
...
func main() {
	...
	cfg := config.LoadServiceConfig("account-service", 8083, 8183)
	...
	accountRepo := repository.NewCassandraAccountRepository(session)

	producer, err := events.NewKafkaProducer(cfg.Kafka.Brokers, "nexora.account.events", logger)
	if err != nil {
	...
	accountService := service.NewAccountService(accountRepo, producer, logger)

	handlers := transport.NewHandlers(accountService, logger)
	...
	router.Use(telemetry.HTTPMetrics(metricsRegistry, "account-service"))
