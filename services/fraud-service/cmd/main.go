...
import (
	...
	"github.com/nexora/nexora/services/fraud-service/internal/transport"
...
func main() {
	...
	cfg := config.LoadServiceConfig("fraud-service", 8089, 8189)
	...
	fraudRepo := repository.NewCassandraFraudRepository(session)

	producer, err := events.NewKafkaProducer(cfg.Kafka.Brokers, "nexora.fraud.events", logger)
	if err != nil {
	...
	fraudService := service.NewFraudService(fraudRepo, producer, logger)
	...
	router.Use(telemetry.HTTPMetrics(metricsRegistry, "fraud-service"))
