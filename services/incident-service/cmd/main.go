...
func main() {
	...
	cfg := config.LoadServiceConfig("incident-service", 8097, 8197)
	...
	incidentRepo := repository.NewCassandraIncidentRepository(session)

	producer, err := events.NewKafkaProducer(cfg.Kafka.Brokers, "nexora.incident.events", logger)
	if err != nil {
	...
	incidentService := service.NewIncidentService(incidentRepo, producer, logger)
